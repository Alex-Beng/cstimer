...
var timer = execMain(function(regListener, regProp, getProp, pretty, ui, pushSignal) {
	...
	function reset() {
		...
		timer.virtual.reset();
		timer.remotecube && timer.remotecube.setEnable(type == 'r');
	...
	function onkeydown(e) {
		...
		switch (getProp('input')) {
			...
			case 'g':
				...
				break;
			case 'r':
				timer.remotecube && timer.remotecube.onkeydown(keyCode);
				break;
		}
	}
	...
	function onkeyup(e) {
		...
		switch (getProp('input')) {
			...
			case 'g':
				...
				break;
			case 'r':
				timer.remotecube && timer.remotecube.onkeyup(keyCode);
				break;
		}
	}
	...
	$(function() {
		...
		regListener('timer', 'property', function(signal, value) {
			if (value[0] == 'timerSize') {
				...
				timer.remotecube && timer.remotecube.setSize(value[1]);
			}
			...
			if (value[0] == 'input') {
				...
				timer.remotecube && timer.remotecube.setEnable(value[1]);
		...
		regProp('vrc', 'vrcMP', 1, PROPERTY_VRCMP, ['n', ['n', 'cfop', 'fp', 'cf4op', 'cf4o2p2', 'roux', 'cll', 'ortega'], PROPERTY_VRCMPS.split('|')], 1);
		...
		regProp('timer', 'input', 1, PROPERTY_ENTERING, ['t', ['t', 'i', 's', 'm', 'v', 'g', 'q', 'b', 'l', 'r'], PROPERTY_ENTERING_STR.split('|')], 1);
